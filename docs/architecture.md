...
이 레이어 구조는 Feature-Sliced Design(FSD)의 기본 분리 원칙을 참고하되,
키오스크 환경의 상태 중심 흐름에 맞게 규칙을 단순화하여 적용했습니다.
...
- 이러한 의존 방향 제한은, 장애 상황에서도 상위 레이어가 하위 구현 변화에 휘둘리지 않도록 하기 위한 선택입니다.
